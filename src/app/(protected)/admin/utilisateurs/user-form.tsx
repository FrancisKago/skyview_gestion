...
export function UserForm({ initial }: { initial?: UserFormInitial }) {
  ...
  // Le mot de passe n'est jamais renvoyé par l'action : il faut le retaper.
  ...
  return (
    <form key={state.attempt ?? 0} action={action} className="bg-card border border-line rounded-xl p-4 grid grid-cols-2 gap-2 text-sm">
      ...
      <Input name="name" placeholder="Nom complet *" defaultValue={v ? v.name : initial?.name} required />
      {initial ? (
        <p className="text-muted text-sm self-center">@{initial.username}</p>
      ) : (
        <Input name="username" placeholder="Identifiant *" defaultValue={v?.username} required />
      )}
      {initial ? (
      ...
      <Select name="role" defaultValue={v ? v.role : initial?.role} required>
